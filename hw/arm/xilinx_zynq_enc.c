...
static void zynq_write_board_setup_enc(ARMCPU *cpu,
...
static void gem_init_enc(NICInfo *nd, uint32_t base, qemu_irq irq)
{
    DeviceState *dev;
    SysBusDevice *s;
...
static inline void zynq_init_spi_flashes_enc(uint32_t base_addr, qemu_irq irq,
                                         bool is_qspi)
{
    DeviceState *dev;
    SysBusDevice *busdev;
...
static void zynq_init_enc(MachineState *machine)
{
    ...
    zynq_init_spi_flashes_enc(0xE0006000, pic[58-IRQ_OFFSET], false);
    zynq_init_spi_flashes_enc(0xE0007000, pic[81-IRQ_OFFSET], false);
    zynq_init_spi_flashes_enc(0xE000D000, pic[51-IRQ_OFFSET], true);
    ...
    gem_init_enc(&nd_table[0], 0xE000B000, pic[54-IRQ_OFFSET]);
    gem_init_enc(&nd_table[1], 0xE000C000, pic[77-IRQ_OFFSET]);
...
static void zynq_machine_enc_init(MachineClass *mc)
{
    mc->desc = "Xilinx Zynq Platform Baseboard for Cortex-A9";
    mc->init = zynq_init_enc;
    mc->max_cpus = 1;
    mc->no_sdcard = 1;
}

DEFINE_MACHINE("xilinx-zynq-a9_enc", zynq_machine_enc_init)
